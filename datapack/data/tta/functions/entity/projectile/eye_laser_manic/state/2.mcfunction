...
execute if score @s monkeyhue.time2 matches 1 facing entity @e[distance=0..40,tag=!tta.ignore,nbt=!{Invulnerable:1b},type=#tta:hostiles,limit=1,sort=nearest] feet rotated ~180 ~ run teleport @s ~ ~ ~ ~ ~
execute if score @s monkeyhue.time2 matches 1 run function tta:entity/projectile/eye_laser_manic/anim/2

execute if score @s monkeyhue.time2 matches 15 run function tta:entity/projectile/eye_laser_manic/projectile/spawn
...
execute if score @s monkeyhue.time2 matches 36.. if entity @e[distance=0..40,tag=!tta.ignore,nbt=!{Invulnerable:1b},type=#tta:hostiles,limit=1] run scoreboard players set @s monkeyhue.entity.AIState 1
execute if score @s monkeyhue.time2 matches 36.. unless entity @e[distance=0..40,tag=!tta.ignore,nbt=!{Invulnerable:1b},type=#tta:hostiles,limit=1] run scoreboard players set @s monkeyhue.entity.AIState 0