...
#
#-------------------------------------------------------------#
execute unless score @s monkeyhue.temp2 matches 26 store result storage tta:root temp.entityAIState.frogzilla int 1 run scoreboard players get @s monkeyhue.entity.AIState
...
execute unless score @s monkeyhue.temp3 matches 1.. if score @s monkeyhue.time3 matches 15 as @e[distance=12..190,type=#tta:frogzilla_preferred,type=!player,sort=random,limit=1,tag=!tta.ignore,tag=!smithed.strict] run function tta:entity/monster/frogzilla2/event/target
execute unless score @s monkeyhue.temp3 matches 1.. if score @s monkeyhue.time3 matches 20 as @e[distance=24..64,type=!#tta:inedible,type=!#tta:explosive,type=!glow_squid,type=!squid,sort=random,limit=1,tag=!tta.ignore,tag=!smithed.strict] run function tta:entity/monster/frogzilla2/event/target
